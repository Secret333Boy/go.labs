...
func HandlePosts(router *httprouter.Router) {
	postsService := posts.GetPostsServiceInstance()
	...
	router.POST("/api/posts", func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
		...
		postsService.AddPost(account, createPostDto.Title, createPostDto.Description)
	...
	router.PATCH("/api/posts/:id", func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		...
		err = postsService.UpdatePost(account, id, createPostDto.Title, createPostDto.Description)
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
	...
	router.POST("/api/posts/:id/messages", func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		...
		err = postsService.AddMessageByPostId(account, id, addMessageDto.Text)
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
		}
	})
	...
	router.PATCH("/api/posts/:id/messages/:messageId", func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		...
		err = postsService.UpdateMessageByPostId(account, id, messageId, addMessageDto.Text)
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
